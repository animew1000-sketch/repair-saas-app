...
// Apply stricter limits before the general API routes.
app.use('/api/login', loginLimiter);
app.use('/api/register', registerLimiter);

// General protection for all API requests.
...
async function initDatabase() {
  const schema = `
    ...
    CREATE TABLE IF NOT EXISTS users (
        ...
        UNIQUE KEY unique_company_user (
          company_id,
          username
        ),

        UNIQUE KEY unique_company_pin (
          company_id,
          pin_code
        )
    );
  ...
  try {
    ...
    try {
      await pool.query(
        'ALTER TABLE companies ADD COLUMN tagline VARCHAR(255) NULL;'
      );
    } catch (e) {}

    try {
      await pool.query(
        'ALTER TABLE companies ADD COLUMN hero_text TEXT NULL;'
      );
    } catch (e) {}
    ...
    await pool.query(`
      ...
      (
        1,
        'Apex Precision Mechanics',
        'apex-mechanics',
        'Precision Auto Service & Complete Repair Workflow',
        'Apex Precision Mechanics provides top-tier diagnostic, engine repair, and routine maintenance solutions.',
        '#f97316'
      ),
      (
        2,
        'Vanguard Auto Performance',
        'vanguard-auto',
        'High Performance Tuning & Custom Mechanical Engineering',
        'Vanguard Auto Performance specializes in dyno tuning, custom exhaust systems, track prep, and high-performance upgrades.',
        '#3b82f6'
      )
...
app.listen(
  ...
  async () => {
    ...
    console.log(
      `Multi-Tenant SaaS Application live on http://localhost:${PORT}`
    );
  }
);